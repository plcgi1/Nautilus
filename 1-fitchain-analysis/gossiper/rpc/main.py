...
from .modules import Fit, RPCModule
import sys
...
class RPCDispatcher:
    ...
    async def _get_result(self, request, debug=False):
        '''
        ...
        try:
            ...
            # log.debug('Method to be called soon %s (params=%s)', method, params)
            # call RPC method
            result = await method(params)  # method(*params)
            # log.debug('from _get_result result=%s', result)
            ...
            log.debug("Validation error while executing RPC method", exc_info=True)
            return None, exc

        except Exception as exc:
            log.info("RPC method caused exception", exc_info=True)
            if debug:
    ...
    async def handle_request(self, request):
        ...
        response = await self.execute(json_request)
        response = json.loads(response)
        # log.debug('from handle_request response=%s', response)
