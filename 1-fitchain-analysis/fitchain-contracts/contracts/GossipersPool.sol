...
contract GossipersPool {
    ...
    function getAvailableGossipers() public view returns(address[] memory){
    ...
    function getChannelGossipers(bytes32 channelId) public view returns(address[] memory) {
    ...
    function getProof(bytes32 channelId) public view returns(bool, bytes32, bytes32[] memory) {
    ...
    function isValidSignature(bytes32 hash, bytes memory signature, address gossiper) private pure returns (bool){
        return (gossiper == ECDSA.recover(hash, signature));
    }

    function submitProof(bytes32 channelId, string memory eot, bytes32[] memory merkleroot, bytes memory signature, bytes32 result) public canVerify(channelId) returns(bool) {
